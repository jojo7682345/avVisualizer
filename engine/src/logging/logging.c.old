...
uint AV_LOG_SUCCESS = AV_LOG_SUCCESS_DEFAULT;
...
const AvLogSettings avLogSettingsDefault = {
    ...
    .printSuccess = AV_LOG_SUCCESS_DEFAULT,
...
void setLogSettings(AvLogSettings settings) {
    AV_LOG_SUCCESS = settings.printSuccess;
...
bool32 checkCategoryDisabled(const char* category) {
...
bool32 checkMessageDisabled(AvResult result) {
    ...
    if(result==AV_SUCCESS && !AV_LOG_SUCCESS){
        return true;
    }
	return false;
}
...
void logDeviceValidation(const char* renderer, AvValidationLevel level, ValidationMessageType type, const char* message, ...) {
	if (AV_LOG_LEVEL > level) {
		return;
	}
    va_list args;
    va_start(args, message);

	avStringPrintf(AV_CSTR(COLOR"["COLOR"%s"COLOR"]"), AV_COLOR_RESET, AV_COLOR_CYAN, renderer, AV_COLOR_RESET);

	switch (type) {
	case VALIDATION_MESSAGE_TYPE_DEVICE_ADDRESS:
		avStringPrintf(AV_CSTRA(COLOR"["COLOR"address"COLOR"]"), AV_COLOR_RESET, AV_COLOR_YELLOW, AV_COLOR_RESET);
		break;
	case VALIDATION_MESSAGE_TYPE_GENERAL:
		avStringPrintf(AV_CSTRA(COLOR"["COLOR"general"COLOR"]"), AV_COLOR_RESET, AV_COLOR_CYAN, AV_COLOR_RESET);
		break;
	case VALIDATION_MESSAGE_TYPE_VALIDATION:
		avStringPrintf(AV_CSTRA(COLOR"["COLOR"validation"COLOR"]"), AV_COLOR_RESET, AV_COLOR_RED, AV_COLOR_RESET);
		break;
	case VALIDATION_MESSAGE_TYPE_PERFORMANCE:
		avStringPrintf(AV_CSTRA(COLOR"["COLOR"performance"COLOR"]"), AV_COLOR_RESET, AV_COLOR_BLUE, AV_COLOR_RESET);
		break;
	}
...
void printTags(AvResult result, AV_LOCATION_ARGS, AV_CATEGORY_ARGS, const char* msg) {
	...
	if (AV_LOG_TIME) {
		...
		avStringPrintf(AV_CSTRA("[%s]"), time);
	}
	if (AV_LOG_TYPE) {
		//level
		...
		avStringPrintf(AV_CSTRA("["COLOR"%s"COLOR"]"), color, result_level, AV_COLOR_RESET);
	}
	if (AV_LOG_CODE) {
		const char* color = "";
		switch (error_type) {
		...
		avStringPrintf(AV_CSTRA("[%s%s"COLOR"]"), color, error_code, AV_COLOR_RESET);
	}
	if (AV_LOG_PROJECT) {
		avStringPrintf(AV_CSTRA("[%s v%i.%i]"), AV_LOG_PROJECT_NAME, AV_LOG_PROJECT_VERSION >> 16, AV_LOG_PROJECT_VERSION & 0xffff);
	}
	if (AV_LOG_FUNC) {
		avStringPrintf(AV_CSTRA("[func: %s]"), func);
	}
	if (AV_LOG_LINE) {
		avStringPrintf(AV_CSTRA("[line %llu]"), line);
	}
	if (AV_LOG_FILE) {
		avStringPrintf(AV_CSTRA("[file: %s]"), file);
	}
	if (AV_LOG_CATEGORY_) {
		avStringPrintf(AV_CSTRA("[category: %s]"), category);
	}
	if (AV_LOG_ERROR && (!unknownCode || !AV_LOG_CODE)) {
		avStringPrintf(AV_CSTRA(" %s"), message);
	}
...
void avLog_(AvResult result, AV_LOCATION_ARGS, AV_CATEGORY_ARGS, const char* msg, ...) {
	...
	avStringPrintfVA(AV_CSTR(msg), args);
    avStringPrintln(AV_EMPTY_STRING);
    va_end(args);
...
void avAssert_(AvResult result, AvResult valid, AV_LOCATION_ARGS, AV_CATEGORY_ARGS, const char* msg, ...) {
	...
	bool32 enabled = !checkCategoryDisabled(category) && !checkMessageDisabled(result);
    va_list args;
    va_start(args, msg);
	...
	if ((result >= AV_LOG_LEVEL || result >= AV_ASSERT_LEVEL) && enabled) {
		printTags(result, line, file, func, category, msg);
		if (AV_LOG_ASSERT) {
			avStringPrintf(AV_CSTRA(COLOR"assert"COLOR" -> "), AV_COLOR_YELLOW, AV_COLOR_RESET);
		}
		avStringPrintfVA(AV_CSTR(msg), args);
        avStringPrintln(AV_EMPTY_STRING);
	}
