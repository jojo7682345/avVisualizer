...
Entity cmdEntityCreate(Scene scene, CommandBuffer* buffer){
    // no commands need to be submitted as id allocation is immediate and has no further side effects.
    // as long as someone has an id, we know that is has been created
...
void cmdEntityDestroy(Scene scene, CommandBuffer* buffer, Entity entity){
    if(avThreadGetID()!=AV_MAIN_THREAD_ID){
        avError("Can only destroy entities from main thread");
        return;
    }
    Command command = {
        .entityId = entity,
...
void applyCommandBuffers(Scene scene, CommandBuffer* buffers, uint32 threadCount){
    ...
    while(headCount > 0){
        ...
        if(staged){
            ...
            performConstructorMasked(scene, ENTITY_LOCAL_INDEX(localEntity), chunk, type, addComponents, )
        }
